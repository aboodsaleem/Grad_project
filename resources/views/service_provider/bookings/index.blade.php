    ...
    @forelse ($bookings as $booking)
      <div class="booking-row d-grid align-items-center grid tx-dark" data-status="{{ $booking->status }}">
        ...
        <div>
          ...
          @elseif(!empty($booking->booking_time))
        ...
        <div>
          <span class="status fw-medium text-center {{ $booking->status }}">{{ ucfirst($booking->status) }}</span>
        </div>
        <div>
          @if ($booking->status === 'pending')
            <form action="{{ route('provider.bookings.accept', $booking->id) }}" method="POST" class="d-inline">@csrf
              <button class="btn btn-success btn-sm">Accept</button>
            </form>
            <form action="{{ route('provider.bookings.reject', $booking->id) }}" method="POST" class="d-inline">@csrf
              <button class="btn btn-danger btn-sm">Reject</button>
            </form>
          @elseif ($booking->status === 'confirmed')
            <form action="{{ route('provider.bookings.complete', $booking->id) }}" method="POST" class="d-inline">@csrf
